...
/* 9.  Recordando objetos */
  ...
  carritoCompras.push(producto1,producto2,producto3,producto1,producto3,producto2);
  ...
  const {precio}= producto3;
  console.log(precio);
  /* destructuring arrays */
  const weekDays = ["lunes","martes","miercoles","jueves", "viernes"];
  const [primero,,third,...restOfDays] =weekDays;
  console.log(primero,third);
  console.log(restOfDays);

  //11.  Iterar un array con objetos como elementos
 
  //11.1 Iterar con FOR  
  for (let z = 0; z < carritoCompras.length; z++) {
    console.log(`nombre: ${carritoCompras[z].nombre}-${carritoCompras[z].precio}`);
    
  }
  //11.2 Iterar con ForEach 
  carritoCompras.forEach((iterador)=>{
    console.log(`Nombre con foreach: ${iterador.nombre}- precio: ${iterador.precio}`);
  })
  //11.3 Iterar con Map  
  carritoCompras.map((iterador)=>{
    console.log(`Nombre con map: ${iterador.nombre}- precio: ${iterador.precio}`);
  })
  //12. FUNCIONES */
  /* ------------------------------------------------------------------------------------------------- */
  ...
  cicloVidaSoftware("Planificacion");

  function cicloVidaSoftware(Planificacion){
    console.log(`la primera fase del ciclo de la vidad de software es: ${Planificacion}`);
    analisis("Analisis");
  }
  function analisis(analisis){
    console.log(`la fase del segundo ciclo de vida es: ${analisis}`);
  }
  
  /* 12.4 Agregar funciones a objetos - metodos de propiedad */
  const riwi={
    leer: function(libro){
      console.log(`los coders de Van Rossum estan leyendo el libro de: ${libro}`);
    },
    escribir: function(texto){
      console.log(`los coders de Van Rossum estan escribiendo un blog sobre: ${texto}`);
    }
  }

  riwi.leer("JavaScript ");
  riwi.escribir("Inteligencia Artificial");


  /* 12.5 Arrow functions */
  
  const verTv = (amor)=>{
    console.log(`la tv se disfruta en las noches con mi ${amor}`);
  }
  verTv("Luisa");

  const verTv1 = amor=>console.log(`la tv se disfruta en las noches comiendo con ${amor}`)
  verTv1("Luisa");
  ...
  /* de una sola linea */
  const verTv2 = amor=>`la tv se disfruta en las noches luedo de hacer el delicioso con ${amor}`;
  console.log(verTv2("Luisa"));
