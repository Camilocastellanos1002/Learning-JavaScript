...
document.addEventListener('DOMContentLoaded',()=>{
    arraysCard = JSON.parse(localStorage.getItem('hire'));
    injectingCoderHtml();
    showCoders(coders);
...
const deleteListCards=document.querySelector('#ListCards');
const deleteAllCards = document.querySelector('#CleanCard');

/*event listeners */
cards.addEventListener('click',selectCards);
deleteListCards.addEventListener('click',deleteCard);
deleteAllCards.addEventListener('click',deteleAll)

function selectCards (e){
    e.preventDefault();
    if(e.target.classList.contains('boton')){
        const selectCard = e.target.parentElement.parentElement;       
...
function deleteCard(e){
    if (e.target.classList.contains('deleteCards')){
        ...
        // console.log(coderToErase); //id 
        arraysCard = arraysCard.filter((ce)=>ce.id !== coderToErase);
        injectingCoderHtml();
    } 
}
...
function clearHtml(){
    tbody_hire.innerHTML="";
}


/*--------------------------LocalStorage--------------------------------------- */
function addStorage(){
    localStorage.setItem('hire',JSON.stringify(arraysCard));
}
